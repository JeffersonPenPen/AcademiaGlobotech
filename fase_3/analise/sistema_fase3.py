...
class SistemaAnaliseEngajamento:
    ...
    def processar_interacoes(self):
        ...
        while not self._fila_interacoes.esta_vazia():
            ...
            try:
                ...
                comment_text = linha.get("comment_text", "") # Corrected: Get 'comment_text'
                comentario = str(comment_text).strip() if comment_text else "" # Corrected: Use 'comment_text'
    ...
    def relatorio1_top_conteudos_por_consumo(self, top_n=10):
        ...
        ordenados = sorted(todos, key=lambda c: c.calcular_total_consumo(), reverse=True)
    ...
    def relatorio2_usuarios_mais_engajados(self, top_n=10):
        print("\n->-> RELATÓRIO 2: Usuários com Mais Interações <-<-\n")
        todos = self._usuarios.percurso_em_ordem()
        ordenados = sorted(todos, key=lambda u: u.total_interacoes(), reverse=True)
        for i, usuario in enumerate(ordenados[:top_n], 1):
            print(f"{i}. {usuario.id_usuario} - {usuario.total_interacoes()} interações")

    def relatorio3_engajamento_por_plataforma(self):
        print("\n->-> RELATÓRIO 3: Total de Interações por Plataforma <-<-\n")
        for nome, plataforma in self._plataformas.items():
            print(f"{nome}: {plataforma.total_interacoes()} interações")

    def relatorio4_conteudos_mais_comentados(self, top_n=10):
        print("\n->-> RELATÓRIO 4: Conteúdos Mais Comentados <-<-\n")
        todos = self._conteudos.percurso_em_ordem()
        ordenados = sorted(todos, key=lambda c: c.contar_comentarios(), reverse=True)
        for i, conteudo in enumerate(ordenados[:top_n], 1):
